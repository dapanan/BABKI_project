...
class SoundManager:
    def __init__(self) -> None:
        ...
        # === НОВОЕ: Защита от спама звуков ===
        self._last_play_time = 0
        self._cooldown = 0.15  # 150 мс кулдаун
    ...
    def load_all(self) -> None:
        if platform.system() == "Emscripten":
            ...
            if not os.path.exists(os.path.join(base_dir, "view")):
                base_dir = os.getcwd()

        base_sound_dir = os.path.join(base_dir, "view", "sounds")
        print(f"DEBUG SoundManager: Loading sounds from {base_sound_dir}")

        # ... (далее код загрузки без изменений, я его сократил для читаемости, копируй свой старый код загрузки, но вставь новые методы ниже)
        ...
        self.lucky_fail = self._load_sound_safe(base_sound_dir, "lucky_coin", "lucky_coin_fail")
    ...
    # === ИЗМЕНЕННЫЕ МЕТОДЫ PRO.play ===

    def play_toss(self, coin_type: str) -> None:
        if self.muted: return
        # Защита от спама
        if time.time() - self._last_play_time < self._cooldown: return
    ...
    def _pick_sound(self, pool: list, last_sound: any) -> any:
        ...
        if not candidates: return random.choice(pool)
